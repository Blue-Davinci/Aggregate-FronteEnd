<script>
    ...
    import { ChevronDown, ChevronUp, AlertTriangle, Trash2, CheckCircle, XCircle, RefreshCw } from 'lucide-svelte';
        ...
        {#each subscriptions as subscription}
            <tr class="cursor-pointer transition-colors hover:bg-purple-100 dark:hover:bg-purple-900">
                ...
                <td class="px-4 py-2 flex items-center space-x-2">
                    {#if subscription.payment_history.subscription.status === 'active'}
                        <CheckCircle class="text-green-500 h-5 w-5" />
                        <span class="text-green-600 dark:text-green-400">Active</span>
                    {:else if subscription.payment_history.subscription.status === 'renewed'}
                        <RefreshCw class="text-blue-500 h-5 w-5" />
                        <span class="text-blue-600 dark:text-blue-400">Renewed</span>
                    {:else if subscription.payment_history.subscription.status === 'expired'}
                        <XCircle class="text-red-500 h-5 w-5" />
                        <span class="text-red-600 dark:text-red-400">Expired</span>
                    {:else if subscription.payment_history.subscription.status === 'canceled'}
                        <XCircle class="text-gray-500 h-5 w-5" />
                        <span class="text-gray-600 dark:text-gray-400">Canceled</span>
                    {/if}
                </td>
                <td class="px-4 py-2">
                    {subscription.payment_history.subscription.price} {subscription.payment_history.currency}
                </td>
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <p class="mt-2 text-sm font-medium text-gray-600 dark:text-gray-300">
                                    ...
                                    {subscription.payment_history.payment_method} ending in {subscription.payment_history.card_last4}
                                </p>
                                <p class="mt-2 text-sm font-medium text-gray-600 dark:text-gray-300">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-600 dark:text-gray-300">
                                    <span class="font-semibold text-gray-800 dark:text-gray-100">Expiration year:</span>
