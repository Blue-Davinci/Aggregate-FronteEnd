<script>
	...
	import {
		adminDeleteFeed,
		adminUpdateFeed
	} from '$lib/dataservice/admin/adminFeedsDataService.js';
	...
	{#each feeds as { feed, feed_user, approval_status, follow_count, isExpanded, priority }, i}
		...
		>
			...
			>
				...
				<button
					class="mt-4 flex items-center justify-center text-xs font-semibold text-blue-500 dark:text-blue-300"
					on:click={() => togglePanel(i)}
				>
					<span>{isExpanded ? 'Hide Actions' : 'Show Actions'}</span>
					{#if isExpanded}
						<ChevronUp class="ml-1 h-4 w-4" /> <!-- Chevron up next to the text -->
					{:else}
						<ChevronDown class="ml-1 h-4 w-4" /> <!-- Chevron down next to the text -->
					{/if}
				</button>
			</div>
		...
		{#if isExpanded}
			...
			>
				<div class="flex justify-center space-x-4">
					...
					<Adminupdatefeedmodal {feed} {form} on:feedUpdated={handleUpdate} />
					<Priorityselector
						feedId={feed.id}
						currentPriority={priority}
						on:onsetpriority={handlePrioritySelect}
					/>
				</div>
			</div>
		{/if}
