...
<script>
    ...
    export let posts;
    //console.log("Posts in Carousel: ", posts);
...
<div class="w-full mt-8 mb-8">
    ...
    <Carousel.Root plugins={[plugin]} class="w-full">
        <Carousel.Content class="flex w-full">
            {#each slicedPosts as post (post.feed.id)}
                    ...
                    <div class="p-1 h-full">
                        <Card.Root>
                            <Card.Content class="flex flex-col h-80 items-center justify-center p-6 relative">
                                <img src={post.feed.Channel.Item[0].ImageURL} alt="Post" class="w-full h-full object-cover mb-2" />
                                <div class="absolute bottom-0 left-0 right-0 bg-gray-900 bg-opacity-75 text-white p-2">
                                    <h3 class="text-lg font-semibold truncate">{post.feed.Channel.Item[0].Title}</h3>
                                    <p class="text-sm truncate">{post.feed.Channel.Item[0].Description.slice(0, 50)}...</p>
                                </div>
                            </Card.Content>
                        </Card.Root>
                    </div>
