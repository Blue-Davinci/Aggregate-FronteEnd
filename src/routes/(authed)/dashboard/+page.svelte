<script>
	...
	let feedID = '';
	// Page details
	$: currentPage = data?.posts?.metadata.current_page ?? 1;
...
{:else}
	...
	{:else}
		<div class="mt-10 flex h-full flex-col items-center justify-center text-center">
			{#if searchQuery !== ''}
				<Bird class="h-20 w-20 text-blue-500" />
				<p class="mt-2 text-lg font-semibold">
					No items found for your search. Try a different keyword!
				</p>
			{:else if feedID !== ''}
				<MessageCircleX class="h-20 w-20 text-red-500" />
				<p class="mt-2 text-lg font-semibold">
					Seems like your filter is too exclusive for our feeds. Maybe loosen the tie a bit?
				</p>
			{:else}
				<Squirrel class="h-20 w-20 text-yellow-500" />
				<p class="mt-2 text-lg font-semibold">
					No posts here! Looks like your feed is on a diet. Head over to the <a
						href="/feeds"
						class="text-blue-500 underline">Feeds</a
